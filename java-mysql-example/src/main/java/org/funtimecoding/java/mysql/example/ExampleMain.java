...
public class ExampleMain {
	...
	private void run() throws SQLException, ClassNotFoundException {
		...
		Connection connection = DriverManager.getConnection(
				"jdbc:mysql://172.17.251.110:3306/example?user=rdurgam&password=Welcome1");
		Statement statement = connection.createStatement();
		ResultSet resultSet = statement.executeQuery("SELECT * FROM example.example");
		...
		if (!exampleUserFound) {
			...
			PreparedStatement preparedStatement = connection
					.prepareStatement("INSERT INTO example.user (name, password, created_at) VALUES (?, ?, ?)");
